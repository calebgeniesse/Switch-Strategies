...
class Strategy(strategy_template.Strategy):
    def __init__(self):
        ...
        self.title_ = "[Strategy Market] [Switch] Entropy in MS2/aptamer switches"
        self.author_ = "Eli Fisker"

        # URL where the strategy was initially submitted
        self.url_ = "https://getsatisfaction.com/eternagame/topics/-strategy-market-switch-entropy-in-ms2-aptamer-switches"
    ...
    def score(self, design, params):
        ...
        for n in range(0, len(states)):
            ...
            if amount != -1:
                ...
                if params[0 + amount] < maxentropy < params[1 + amount]:
                    score += params[4 + amount]
                elif params[2 + amount] < maxentropy < params[3 + amount]:
                    score += params[5 + amount]

            if maxentropy < params[12]:
                score += params[16]
            elif params[13] < maxentropy < params[14]:
                # Linear score of reward from +1 to 0 if closer to 2.1 then 3
                score += ((maxentropy - params[13]) * (params[17] - params[18]))
            elif maxentropy > params[15]:
                # Exponential penalty of A * (B^(entropy above 3)), B = 2, A = 1 (default)
                score += (params[20]) * pow((params[19]), (maxentropy - params[15]))
